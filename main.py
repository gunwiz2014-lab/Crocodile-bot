...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Магазин ──
        ...
        if key=="vip":
        ...
        elif key=="extra_life": p["total_score"]-=price; p["extra_life"]=True; await query.answer("❤️ Жизнь!",show_alert=True)
        elif key=="time_bonus": p["total_score"]-=price; p["time_bonus"]=True; await query.answer("⏱ +30 сек!",show_alert=True)
        elif key=="double_daily": p["total_score"]-=price; p["double_daily"]=True; await query.answer("🎁 x2 бонус!",show_alert=True)
        elif key=="score_boost": p["total_score"]-=price; p["score_boost"]=True; await query.answer("⚡ x1.5!",show_alert=True)
        elif key=="multiplier": p["total_score"]-=price; p["multiplier"]=True; await query.answer("✖️ x3!",show_alert=True)
        elif key=="lottery":
            ...
            if players[target].get("shield_until") and date.today().isoformat()<=players[target]["shield_until"]:
                await reply(f"🧪 Яд отражён щитом *{tname}*!", parse_mode="Markdown")
            else:
                players[target]["total_score"]=max(0,players[target]["total_score"]-200)
                await reply(f"🧪 *{user.first_name}* травит *{tname}*! -200 оч!", parse_mode="Markdown")
        elif key=="steal":
            victims=[uid for uid in players if uid!=user.id and players[uid]["total_score"]>=50]
            if not victims: await query.answer("Нет богатых!",show_alert=True); return
            p["total_score"]-=price; target=random.choice(victims); tname=players[target]["name"]
            if players[target].get("shield_until") and date.today().isoformat()<=players[target]["shield_until"]:
                await reply(f"🛡 *{tname}* защищён! Кража не удалась.", parse_mode="Markdown")
            else:
                ...
                await reply(f"💰 *{user.first_name}* грабит *{tname}*! +{stolen} оч!", parse_mode="Markdown")
        elif key=="freeze":
            victims=[uid for uid in players if uid!=user.id]
            if not victims: await query.answer("Нет игроков!",show_alert=True); return
            p["total_score"]-=price; target=random.choice(victims); tname=players[target]["name"]
            players[target]["frozen_until"]=(date.today()+timedelta(days=1)).isoformat()
            await reply(f"❄️ *{user.first_name}* замораживает *{tname}* на 24ч!", parse_mode="Markdown")
        elif key=="mirror": p["total_score"]-=price; p["mirror"]=True; await query.answer("🪞 Зеркало!",show_alert=True)
        elif key=="shield": p["total_score"]-=price; p["shield_until"]=(date.today()+timedelta(days=1)).isoformat(); await query.answer("🛡 Щит на 24ч!",show_alert=True)
        elif key=="mystery_box":
            p["total_score"]-=price
            available=[k for k in SHOP_ITEMS if k not in ("mystery_box","vip","speedrun","vip_pack","bar_vip")]
            pk=random.choice(available); pi,pn,_,_=SHOP_ITEMS[pk]
            p.setdefault("inventory",{})[pk]=p.get("inventory",{}).get(pk,0)+1
            await reply(f"📦 *{user.first_name}* открывает ящик!\n\n🎉 Выпало: {pi} *{pn}*!", parse_mode="Markdown")
        elif key=="fortune":
            p["total_score"]-=price
            hints=["🔥 Следующая буква есть в слове!","💀 Буква А или Е — нет в слове!","⭐ В слове больше 5 букв!","🌙 Первая буква — согласная!","✨ В слове есть удвоенная буква!"]
            await reply(f"🔮 *Фортуна:*\n\n{random.choice(hints)}", parse_mode="Markdown")
        elif key=="crown":
            p["total_score"]-=price; p["crown_until"]=(date.today()+timedelta(days=1)).isoformat()
            await reply(f"💎 *{user.first_name}* получает корону на 24ч! 👑", parse_mode="Markdown")
            ...
            p["total_score"]-=price
            await reply(f"🚀 *{user.first_name}* в топе!\n\n📊 {global_scores_text()}", parse_mode="Markdown")
        elif key=="farm_boost":
            p["total_score"]-=price; p["farm_boost_until"]=(date.today()+timedelta(days=1)).isoformat()
            await reply(f"🌾 *{user.first_name}* — бустер фермы x2 на 24ч!", parse_mode="Markdown")
        elif key=="energy_pack":
            p["total_score"]-=price; p["score_boost"]=True
            p.setdefault("inventory",{})["energy_pack"]=p.get("inventory",{}).get("energy_pack",0)+10
            await reply(f"⚡ *{user.first_name}* — энергопак! 10 энергетиков + бустер!", parse_mode="Markdown")
        elif key=="sale_pack":
            p["total_score"]-=price; p["protection"]=True; p["double"]=True; p["extra_life"]=True
            await reply(f"🎉 *{user.first_name}* — пакет скидок!\n🛡 Защита + 🎯 Удвоение + ❤️ Жизнь!", parse_mode="Markdown")
        elif key=="starter_pack":
            p["total_score"]-=price; p["score_boost"]=True
            for _ in range(3): p.setdefault("inventory",{})["lottery"]=p.get("inventory",{}).get("lottery",0)+1
            await reply(f"🎯 *{user.first_name}* — стартовый пакет!\n🎟 Лотерея x3 + ⚡ Бустер!", parse_mode="Markdown")
        elif key=="vip_pack":
            p["total_score"]-=price; p["vip"]=True; p["achievements"].add("vip_buyer")
            p["crown_until"]=(date.today()+timedelta(days=1)).isoformat(); p["nick_color"]="🌈"
            await reply(f"👑 *{user.first_name}* — VIP Пакет!\n👑 VIP + 💎 Корона + 🌈 Радужный ник!", parse_mode="Markdown")
        elif key=="sabotage":
            game=wheel_games.get(chat_id)
            if not game or not game.get("active"): await query.answer("Нет Поля чудес!",show_alert=True); return
            p["total_score"]-=price
            ...
            else: await query.answer("Нет других!",show_alert=True)
...
async def message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_frozen(user.id): return

    # ── Математика ──
    if ctx.chat_data.get("math_active"):
        if text_lower==ctx.chat_data.get("math_answer",""):
            ctx.chat_data["math_active"]=False
            add_score(user.id,user.first_name,80); p["wins"]+=1
            await update.message.reply_text(f"🧮 *{user.first_name}* правильно! +80 оч\n\n📊 {global_scores_text()}", parse_mode="Markdown", reply_markup=make_main_keyboard(group=g))
        return
    ...
    if chat_id in uno_games and uno_games[chat_id].get("active"):
        ...
        curr_id=game["players"][game["current"]%len(game["players"])][0]
        if user.id!=curr_id: return
        hand=game["hands"].get(user.id,[])
        if text in hand:
            top=game["top_card"]
            if uno_card_matches(text,top):
                hand.remove(text); game["top_card"]=text
                if not hand:
                    ...
                    if "🔄" in text: game["direction"]*=-1; next_idx=(game["current"]+game["direction"])%len(game["players"])
                    if "⛔" in text: next_idx=(next_idx+game["direction"])%len(game["players"])
                    if "➕2" in text:
                        nid=game["players"][next_idx][0]
                        for _ in range(2):
                            if game["deck"]: game["hands"].setdefault(nid,[]).append(game["deck"].pop())
                    if "➕4" in text:
                        nid=game["players"][next_idx][0]
                        for _ in range(4):
                            if game["deck"]: game["hands"].setdefault(nid,[]).append(game["deck"].pop())
                        next_idx=(next_idx+game["direction"])%len(game["players"])
                    game["current"]=next_idx%len(game["players"])
                    next_name=game["players"][game["current"]][1]
                    await update.message.reply_text(f"🎴 *{user.first_name}* играет *{text}*!\n\nВерхняя: {game['top_card']}\nХодит: *{next_name}*\nКарт у {user.first_name}: {len(hand)}", parse_mode="Markdown", reply_markup=make_uno_kb())
            else: await update.message.reply_text(f"❌ Карта *{text}* не подходит!\nВерхняя: {game['top_card']}", parse_mode="Markdown")
        return
    ...
    if chat_id in number_games and number_games[chat_id].get("active"):
        ...
        if text_lower.isdigit():
            guess=int(text_lower); number=game["number"]
    ...
    if chat_id in hangman_games and hangman_games[chat_id].get("active"):
        ...
        if len(text_lower)==1 and text_lower.isalpha():
            if text_lower in guessed: await update.message.reply_text(f"Буква *{text_lower.upper()}* уже была!", parse_mode="Markdown"); return
            guessed.add(text_lower); errors=sum(1 for c in guessed if c not in word)
            game["errors"]=errors; pic=HANGMAN_PICS[min(errors,5)]; shown=display_word(word,guessed); wrong=[c.upper() for c in guessed if c not in word]
            if text_lower in word:
                ...
                else: await update.message.reply_text(f"✅ Буква *{text_lower.upper()}* есть! +50\n\n{pic} Ошибок: {errors}/{max_err}\nСлово: `{shown}`\nНеверные: {' '.join(wrong) if wrong else '—'}", parse_mode="Markdown")
            else:
                if errors>=max_err:
                    if game["timer_task"]: game["timer_task"].cancel()
                    game["active"]=False
                    await update.message.reply_text(f"💀 *{user.first_name}* проиграл(а)!\nСлово: *{word.upper()}*", parse_mode="Markdown", reply_markup=make_main_keyboard(group=g))
                else: await update.message.reply_text(f"❌ Буквы *{text_lower.upper()}* нет!\n\n{pic} Ошибок: {errors}/{max_err}\nСлово: `{shown}`\nНеверные: {' '.join(wrong)}", parse_mode="Markdown")
        return
    ...
    if chat_id in wheel_games and wheel_games[chat_id].get("active"):
        ...
        if len(text_lower)>1:
            ...
            return
        if len(text_lower)==1 and text_lower.isalpha():
            if text_lower in guessed or text_lower in game.get("wrong_letters",set()):
                await update.message.reply_text(f"Буква *{text_lower.upper()}* уже была!", parse_mode="Markdown"); return
            ...
            if text_lower in word:
                count=word.count(text_lower); guessed.add(text_lower)
                ...
                else: await update.message.reply_text(f"✅ *{user.first_name}* открыл(а) *{text_lower.upper()}* ({count}шт) +{points}{dm}{sm}\n\nСлово: `{shown}`", parse_mode="Markdown", reply_markup=make_wheel_kb())
            else:
                game.setdefault("wrong_letters",set()).add(text_lower); p["streak"]=0
                game["round_scores"][user.id]=game["round_scores"].get(user.id,0)-100
                await update.message.reply_text(f"❌ *{user.first_name}*, *{text_lower.upper()}* нет! -100\nСлово: `{display_word(word,guessed)}`", parse_mode="Markdown", reply_markup=make_wheel_kb())
        return
...
def main():
    ...
    for cmd, func in [
        ...
        ("math",cmd_math),("shop",cmd_shop),("join",cmd_join),("stopbot",cmd_stopbot),
    ]:
    ...
    print("🎮 МЕГА БОТ запущен! 15 игр + Бар + Ферма тайкун + 33 предмета!")
